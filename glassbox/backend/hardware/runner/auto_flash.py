...
def verify_post_flash(pico_port: str, *, baud: int = 115200,
    ...
    Inline STATUS probe -- no dependency on `runner.open_pod`. We open the
    Pico's USB CDC and write `STATUS\\n`. The Pico's command parser proxies
    that onto its UART link to the ESP32, waits for the ESP32's reply, and
    forwards it back. So a single round-trip exercises the entire chain:
    ...
    try:
        ...
        # open(); give it a beat, then drop any stale RX (e.g. a leftover
        # READY banner from the post-bridge auto-reboot or a trailing ACK
