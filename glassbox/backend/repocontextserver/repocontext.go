...
func handleCloneRepo(w http.ResponseWriter, r *http.Request) {
    ...
    // Always (re)populate the workspace from scratch. Validation above
    // guarantees the caller supplied exactly one of git_url or path, which means
    // they want THIS repo present right now. Returning "already_cloned: true" on
    // stale leftovers (e.g. a half-cloned .git/ from a prior failure, or a
    // different repo from a prior session) caused silent failures where
    // get_repo_context would walk the old tree and return empty entries.
    if err := os.MkdirAll(repoRoot, 0o755); err != nil {
        http.Error(w, fmt.Sprintf("Failed to create repo dir: %v", err), http.StatusInternalServerError)
        return
    }
    if entries, readErr := os.ReadDir(repoRoot); readErr == nil && len(entries) > 0 {
        log.Printf("clone_repo: wiping %d stale entries in %s before re-clone", len(entries), repoRoot)
        for _, e := range entries {
            if rmErr := os.RemoveAll(filepath.Join(repoRoot, e.Name())); rmErr != nil {
                http.Error(w, fmt.Sprintf("Failed to clear stale workspace: %v", rmErr), http.StatusInternalServerError)
                return
            }
        }
    }
